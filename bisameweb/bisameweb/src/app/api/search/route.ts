...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!listingsBaseUrl) {
      console.error('Missing NEXT_PUBLIC_LISTINGS_BASE_URL environment variable');
      return NextResponse.json(
    ...
    const searchApiUrl = `${listingsBaseUrl}${listingsSearchPath}?page=${page}&query=${encodeURIComponent(query)}&pageSize=${pageSize}`;
    ...
    if (axios.isAxiosError(error)) {
      if (error.response) {
        ...
        return NextResponse.json(
          {
            error: errorData.message || errorData.error || 'Error from search service',
            statusCode: error.response.status,
            details: errorData.details || 'An error occurred while processing your search'
          },
        ...
        return NextResponse.json(
          {
            ...
            message: 'The search service is not responding. Please try again later.'
          },
          { status: 503 }
        );
      } else {
