...
const AccountInfo = () => {
  ...
  const profile = profileData?.profilePicture || data?.info?.profile || '/user.png';
      ...
      <div className="mb-4 text-sm md:text-base">
        <div className="flex items-center mb-4">
          ...
          <p className="font-semibold text-gray-800">
            {loading ? <span className="bg-gray-200 rounded w-24 h-5 inline-block animate-pulse" /> : name}
          </p>
        </div>
        <div className="ml-2 space-y-2">
          ...
          {loading ? (
            <p className="text-gray-600">
              <span className="font-medium text-blue-500">Phone:</span> <span className="bg-gray-200 rounded w-24 h-4 inline-block animate-pulse" />
            </p>
          ) : (
            <p className="text-gray-600">
              <span className="font-medium text-blue-500">Phone:</span> {phone}
            </p>
          )}
          ...
          {!loading && profileData && profileData.userReferralCode && (
            <p className="text-gray-600">
              <span className="font-medium text-blue-500">Referral Code:</span> {profileData.userReferralCode}
            </p>
          )}
          ...
          {!loading && profileData && profileData.countryName && (
            <p className="text-gray-600">
              <span className="font-medium text-blue-500">Country:</span> {profileData.countryName}
            </p>
          )}
