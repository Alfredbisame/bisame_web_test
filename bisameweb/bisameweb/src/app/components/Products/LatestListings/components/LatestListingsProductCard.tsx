...
import { LatestListingsProductCardProps } from "../types";

const LatestListingsProductCard = ({
  product,
  index,
  onClick,
}: LatestListingsProductCardProps) => {
  ...
  return (
    <div
      ...
      className="shadow-md p-3 relative transition-all duration-300 hover:bg-gray-100 hover:shadow-md hover:scale-105 cursor-pointer h-auto flex flex-col rounded-md"
    >
      {/* Display NEW ARRIVAL badge */}
      {product.badge && (
        <div className="absolute top-4 right-4 z-10">
          <span
            className={`${product.badge.color} text-white text-xs font-semibold px-1.5 sm:px-2 sm:py-1 rounded text-[10px] sm:text-xs py-1`}
          >
            {product.badge.text}
          </span>
        </div>
...
export default LatestListingsProductCard;
