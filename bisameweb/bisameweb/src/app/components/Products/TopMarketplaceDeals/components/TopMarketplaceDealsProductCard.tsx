...
import { TopMarketplaceDealsProductCardProps } from "../types";
// Removed StarRating import

const TopMarketplaceDealsProductCard = ({
  product,
  index,
  onClick,
}: TopMarketplaceDealsProductCardProps) => {
  ...
  return (
    <div
      ...
      className="shadow-md p-3 relative transition-all duration-300  cursor-pointer h-auto flex flex-col rounded-md"
    >
      {product.isPromoted && (
        <div className="absolute top-4 right-4 z-10">
          <span className="bg-blue-500 text-white text-xs font-semibold px-1.5 sm:px-2 sm:py-1 rounded text-[10px] sm:text-xs py-1">
            PROMOTED
          </span>
        </div>
      )}
...
export default TopMarketplaceDealsProductCard;
