...
const DeclinedProducts = () => {
  ...
  const { data, loading, error } = useMyPostData("Declined");
  const declinedProducts: Product[] = data?.results || [];
    ...
    return (
      <div className="flex flex-col items-center justify-center py-16">
        ...
        <p className="text-gray-500 text-sm">
          Loading your declined products...
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 bg-gradient-to-br from-blue-50 to-white rounded-xl shadow-inner">
        ...
        <h2 className="md:text-xl font-bold text-gray-700 mb-2">
          No Declined Products
        </h2>
        <p className="text-gray-500 text-sm md:text-base max-w-md text-center mb-4">
          You have no declined products at the moment.
          <br />
          Declined products will appear here for your review.
        </p>
        <button
          onClick={() => window.location.reload()}
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5 gap-6 mt-6 ">
        {declinedProducts.map((product) => {
          ...
          return (
            <div
              key={product.id}
              className="border p-2 relative transition-all duration-300 hover:bg-gray-100 hover:shadow-md hover:scale-105 cursor-pointer rounded-md opacity-75"
            >
              {/* Status badge */}
              <div className="absolute top-6 right-4 z-10">
                <span className="bg-red-600 text-white text-xs font-semibold px-2 py-1 rounded-sm shadow">
                  Declined
                </span>
              </div>
              <Image
                ...
                width={150}
                height={150}
                className="w-full h-40 object-cover mt-2 rounded-md transition-transform duration-300 transform grayscale"
