...
// Helper function to get all child categories across all services
export const getAllChildCategories = (
  ...
  return data.flatMap((service) =>
    service.sub.flatMap((subcategory) =>
      subcategory.childcategory.map((child) => ({
        ...child,
        parentSubcategory: subcategory.subcategory,
        parentCategory: service.category,
      }))
    )
  );
...
// Helper function to search child categories by name
export const searchChildCategories = (
  data: ServiceData[] | undefined,
  searchTerm: string
): (ChildCategory & {
  parentSubcategory: string;
  ...
  const term = searchTerm.toLowerCase();
  const allChildCategories = getAllChildCategories(data);

  return allChildCategories.filter(
    (child) =>
      child.childcategory.toLowerCase().includes(term) ||
      child.parentSubcategory.toLowerCase().includes(term) ||
      child.parentCategory.toLowerCase().includes(term)
  );
};
