...
const FollowersFollowing: React.FC = () => {
  ...
  const { data: followersData, loading: followersLoading, error: followersError, mutate: followersMutate } = useFollowersData(page, 10);
  const { data: followingData, loading: followingLoading, error: followingError, mutate: followingMutate } = useFollowingData(page, 10);
  const { handleFollowToggle, followLoading, unfollowLoading } = useFollowUnfollow();

  // Transform data based on active tab
  const followers = followersData?.data?.data?.results?.map(transformUserToProfile) || [];
  const following = followingData?.data?.data?.results?.map(transformUserToProfile) || [];
  ...
  const loading = activeTab === 'followers' ? followersLoading : followingLoading || followLoading || unfollowLoading;
  ...
  if (loading) {
    return (
      <div className="bg-white flex items-center justify-center p-6 mt-6">
  ...
  return (
    <div className="bg-white flex items-center justify-center p-6 mt-6">
      <div className="max-w-7xl w-full border border-gray-200 rounded-md shadow-sm">
        {/* Tab Navigation */}
        <div
          className="border-b border-gray-200 flex text-sm font-normal text-gray-600 select-none relative"
          role="tablist"
        >
          <button
            role="tab"
            ...
            className={`flex-1 py-4 px-2 relative transition-all duration-300 ease-in-out ${
              activeTab === 'followers'
          ...
          >
            ...
            {/* Active indicator */}
            {activeTab === 'followers' && (
              <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 h-0.5 bg-orange-500 rounded-full transition-all duration-300 ease-in-out w-16 sm:w-20 md:w-24"></div>
            )}
          </button>
        ...
        >
          {currentProfiles.length > 0 ? (
            <>
                ...
                >
                  <ProfileCard
                    ...
                    onFollowToggle={(profileId, currentlyFollowing) => 
                      handleFollowToggle(profileId, currentlyFollowing, followersMutate, followingMutate)
                    }
              ...
              {/* Pagination */}
              <Pagination 
                currentPage={activeTab === 'followers' ? followersCurrentPage : followingCurrentPage}
                totalPages={activeTab === 'followers' ? followersTotalPages : followingTotalPages}
