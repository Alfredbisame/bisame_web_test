  ...
  return (
    <div className="px-3 py-3 md:px-[3%] lg:px-[6%] xl:px-[7%] 2xl:px-[12%] space-y-2 flex flex-col min-h-[80%]">
      <div className="bg-orange-500 flex justify-between items-center px-3 py-2 md:px-4 md:py-2 rounded-lg shadow-md">
        <h1 className="text-white text-sm md:text-base xl:text-lg font-semibold tracking-wide">
          Choose your <span className="capitalize">{formattedCategory}</span>
        </h1>
      ...
      <div className="h-full flex gap-3 ">
        <div className="bg-white w-1/3 md:w-1/4 bg-opacity-90 space-y-1.5 rounded-lg p-2 shadow-md flex flex-col">
          {processedCategories.map(({ name, id }) => (
            ...
            >
              <span
                className={`block text-sm font-semibold text-gray-800 truncate px-2 py-2 rounded-md transition-colors duration-200 group-hover:text-orange-600 ${activeCategory == name ? "bg-gray-100": ""}`}
                title={name}
              >
                {name}
              </span>
        ...
        <div className="flex-grow grid grid-cols-3 md:grid-cols-6 grid-rows-5 gap-3">
          {activeCategory && (
            <SubCategoryCards
